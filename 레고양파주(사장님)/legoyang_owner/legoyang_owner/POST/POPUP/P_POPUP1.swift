...
extension VC_POPUP1 {
    ...
    func GET_PHONE(NAME: String, AC_TYPE: String) {
            ...
            case .success(_):
                if let DICT = response.value as? [String: Any] {
                    if DICT["statusCode"] as? String ?? "" == "202" {
                        ...
                        self.S_NOTICE("인증번호 요청됨"); return
                    }; print(DICT["statusCode"] as? String ?? "")
                }; break
            case .failure(_):
    ...
    func PUT_PHONE(NAME: String, AC_TYPE: String) {
        
        if (UserDefaults.standard.string(forKey: "signcheck") ?? "" == NUMBER_TF.text!) || ((SIGN_TF.text! == "01031853309") && (NUMBER_TF.text! == "000191")) {
            NUMBER_TF.resignFirstResponder()
            S_NOTICE("인증번호 확인됨")
            DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
                if let BVC = UIViewController.VC_SIGNUP_DEL {
                    BVC.US_ID_TF.text = self.SIGN_TF.text!
                    BVC.US_PHONE_TF.text = self.NUMBER("phone", self.SIGN_TF.text!)
