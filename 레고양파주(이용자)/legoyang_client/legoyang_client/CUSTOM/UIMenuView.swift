...
class UIMenuView: UIControl {
    ...
    @IBInspectable var bottomLineThumbViewHeight: CGFloat = 4 { didSet { updateView() } }
    ...
    //Private datasources
    @IBInspectable private var buttonImages: [UIImage] = [] { didSet { updateView() } }
    @IBInspectable private var buttonTitles: [String] = [] { didSet { updateView() } }
    @IBInspectable private var buttonColors: [UIColor] = [] { didSet { updateView() } }
    ...
    @IBInspectable var padding: CGFloat = 0 { didSet { updateView() } }
    @IBInspectable var customBorderWidth: CGFloat = 0 { didSet { layer.borderWidth = customBorderWidth } }
    
    @IBInspectable var customBorderColor: UIColor = .clear { didSet { layer.borderColor = customBorderColor.cgColor } }
    
    //animation duration is 0.3 by default
    @IBInspectable var animationDuration: CGFloat = 0.3
    @IBInspectable var thumbViewAlpha: CGFloat = 1.0 { didSet { thumbView.alpha = thumbViewAlpha } }

    //segmented
    @IBInspectable var segmentedBackGroundColor: UIColor = .clear { didSet { backgroundColor = segmentedBackGroundColor } }
    
    //thumbView
    @IBInspectable var thumbViewColor: UIColor = .darkGray { didSet { updateView() } }
    
    // MARK: SEGMENTED CONTROL WITH TEXT
    @IBInspectable var textColor: UIColor = .lightGray { didSet { updateView() } }
    
    @IBInspectable var selectedTextColor: UIColor = .white { didSet { updateView() } }
    
    public var titlesFont: UIFont? { didSet { updateView() } }
    
    // MARK: SEGMENTED CONTROL WITH IMAGES
    //if images with change on it's tint color on selection
    @IBInspectable var buttonColorForNormal: UIColor = .lightGray { didSet { updateView() } }
    
    @IBInspectable var buttonColorForSelected: UIColor = .darkGray { didSet { updateView() } }
    ...
    @IBInspectable var imageForItemWithDynamicColors: UIImage? { didSet { updateView() } }
    ...
    public var fillEqually: Bool = false { didSet { layoutSubviews() } }
    
    //change the control for images and text
    public var itemsWithText: Bool = false { didSet { updateView() } }
    public var roundedControl: Bool = false { didSet { updateView() } }
    public var bottomLineThumbView: Bool = false { didSet { updateView() } }
    
    public var thumbViewHidden: Bool = false { didSet { thumbView.isHidden = thumbViewHidden } }
    ...
    @IBInspectable var buttonsWithDynamicImages: Bool = false { didSet { updateView() } }
    
    //C) This makes changes on buttons with colors and only if itemsWithText = false
    @IBInspectable public var itemsWithDynamicColor: Bool = false { didSet { updateView() } }
    ...
    func updateSegmentedWith<T>(items: T) {
        ...
        UIView.animate(withDuration: 0.4) { self.updateView(); self.thumbView.alpha = 1 }
    ...
    private func resetViews() { buttons.removeAll(); subviews.forEach { $0.removeFromSuperview() } }
...
//        layer.cornerRadius = roundedControl ? frame.height / 2 : 1.0
        ...
        if selectedSegmentIndex == 0 { setThumbView() }
        //if fillEqually is not true the layout is not in stackview and its set based on frames
        guard !fillEqually else { return }
        for (index, btn) in self.buttons.enumerated() { btn.frame = setFrameForButtonAt(index: index) }
    }
    
    // MARK: THUMBVIEW LAYOUT
    private func setThumbView() {

        let thumbViewHeight = bottomLineThumbView ? bottomLineThumbViewHeight : bounds.height - padding * 2
...
//        thumbView.frame = CGRect(x: thumbViewPositionX, y: thumbViewPositionY, width: thumbViewWidth, height: thumbViewHeight)
        thumbView.frame = CGRect(x: (thumbViewWidth / 2) - (self.bottomLineThumbViewHeight), y: thumbViewPositionY - 5, width: bottomLineThumbViewHeight, height: thumbViewHeight)
        thumbView.layer.cornerRadius = bottomLineThumbViewHeight / 2
//        thumbView.layer.cornerRadius = roundedControl ? thumbViewHeight / 2 : 1.0
    ...
    private func setButtonsWithImages() {
        ...
        for buttonImage in self.buttonImages {
            ...
            button?.imageEdgeInsets = UIMenuView.imageInsets
            button?.addTarget(self, action: #selector(buttonTapped(button:)), for: .touchUpInside)
            buttons.append(button!)
        }
        
        if !buttonsWithDynamicImages { buttons[selectedSegmentIndex].tintColor = buttonColorForSelected }
    }
...
extension UIMenuView {
    
    // MARK: MAIN ACTION: .valueChanged
    fileprivate func performAction() { sendActions(for: .valueChanged) }
    ...
    fileprivate func moveThumbView(at index: Int) {
        ...
        UIView.animate(withDuration: TimeInterval(self.animationDuration), animations: {
            self.thumbView.frame.origin.x = selectedStartPosition + (thumbViewWidth / 2) - (self.bottomLineThumbViewHeight)
        })
    }
