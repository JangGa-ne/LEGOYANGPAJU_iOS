...
extension VC_SEARCH {
    ...
    func GET_STORE(NAME: String, AC_TYPE: String) { print("[\(NAME)], (\(AC_TYPE)) 요청함")
        // 데이터 삭제
        OBJ_STORE.removeAll(); TABLEVIEW.reloadData()
        
        Firestore.firestore().collection(AC_TYPE).addSnapshotListener { response, error in
            
            if let response = response {
                // 데이터 삭제
                self.OBJ_STORE.removeAll()
                ...
                for response in response.documents {
                    ...
                    if (STORE_ID == "01031853309") || (STORE_ID == "01090576393") || (STORE_ID == "01034231219") || (STORE_ID == "01090760335") || (STORE_ID == "01031262497") || (STORE_ID == "01031870005") || (STORE_ID == "01041348066") {
                        if (MEMBER_ID == "01031853309") || (MEMBER_ID == "01090576393") || (MEMBER_ID == "01034231219") || (MEMBER_ID == "01090760335") || (MEMBER_ID == "01031262497") || (MEMBER_ID == "01031870005") || (MEMBER_ID == "01041348066") { self.OBJ_STORE.append(AP_VALUE) }
                    } else {
                        self.OBJ_STORE.append(AP_VALUE)
                    }
                }; self.OBJ_STORE.sort { front, behind in Int(front.USE_ITEM_TIME) ?? 0 > Int(behind.USE_ITEM_TIME) ?? 0 }
